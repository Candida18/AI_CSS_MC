...
class FrequencyReuse(Tk):
	...
	def __init__(self, cluster_size, columns=16, rows=10, edge_len=30):
		...
		self.reuse_xy = []
		self.canvas = Canvas(
			self, width=self.CANVAS_WIDTH, height=self.CANVAS_HEIGHT, bg="#B088FF"
		)
	...
	def create_textbox(self):
		...
		self.canvas.create_window((30, 600), anchor="w", window=txt)
		...
		if self.first_click:
			...
			if curr in self.reuse_list:
				...
				if self.curr_count == len(self.reuse_list) - 1:
					...
					self.write_text(f"Completed in {toc - tic:0.4f} seconds")
					self.show_lines()
				self.curr_count += 1
			else:
				self.write_text(
